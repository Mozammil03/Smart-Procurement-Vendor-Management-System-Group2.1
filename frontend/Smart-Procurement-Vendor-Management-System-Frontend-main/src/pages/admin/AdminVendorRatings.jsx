...
export default function AdminVendorRatings() {
  ...
  const loadRatings = async () => {
    try {
      ...
      // For debugging - show what the error contains
      if (err.response) {
      ...
      setRatings([]); // Set empty array as fallback
    }
  };
  ...
  const submitRating = async (e) => {
    ...
    const payload = {
      vendorId: Number(form.vendorId), // ✅ already the ID from the dropdown
      ratedByUserId: Number(localStorage.getItem("userId") || 1),
